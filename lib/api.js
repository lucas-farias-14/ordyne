...
export const createCode = async (form) => {
    let local_err = false;
    // trim the code before using it
    let upperCaseAndTrimCode = String(form.code).trim();
    upperCaseAndTrimCode = upperCaseAndTrimCode.toUpperCase();
    console.log(upperCaseAndTrimCode);
    try {
        let response = await fetch(`https://api.linketrack.com/track/json?user=teste&token=1abcd00b2731640e886fb41a8a9671ad1434c599dbaa0a0de9a5aa619f29a83f&codigo=${upperCaseAndTrimCode}`);
        if (!response.ok) {
            local_err = true;
            ...
            while (status === 429) {
                ...
                response = await fetch(`https://api.linketrack.com/track/json?user=teste&token=1abcd00b2731640e886fb41a8a9671ad1434c599dbaa0a0de9a5aa619f29a83f&codigo=${upperCaseAndTrimCode}`);
        ...
        if (data && data.eventos && data.eventos.length > 0) {
            await AsyncStorage.setItem(String(upperCaseAndTrimCode), JSON.stringify(data));
        } else {
            local_err = true;
